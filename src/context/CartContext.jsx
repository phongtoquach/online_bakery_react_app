...
export function CartProvider({ children }) {
  ...
  const [cart, setCart] = useState(() => {
    ...
    console.log(
      "[CartProvider] set value ban dau cho bien useState cart - localStorage bakeryCartLs vua lay ra : ",
    );
    ...
    if (cartJsonStr) {
      ...
      console.log(
        "[CartProvider] set value ban dau cho bien useState cart - localStorage bakeryCartLs ton tai ! Convert thanh mang JS !",
      );
      console.log(cartLs);
  ...
  useEffect(() => {
    console.log(
      "[CartProvider] đang chạy useEffect() set localStorage item bakeryCartLs ! value là biến useState cart !",
    );
    console.log(cart);
    ...
    return () => {
      console.log(
        "[CartProvider] đang chạy hàm cleanup của useEffect() set localStorage item bakeryCartLs !",
      );
    };
  }, [cart]);
  ...
  function addProductToCart(productId, quantity = 1) {
    console.log(
      "[addProductToCart] chuan bi add vao cart - product ID : " +
        productId +
        " ; quantity : " +
        quantity,
    );
    ...
    if (foundCartItem) {
      ...
      console.log(
        "[CartProvider - addProductToCart] Product co ID " +
          productId +
          " da co trong cart; Quantity hien la " +
          foundCartItem.quantity +
          ". Nen gio chi can tang quantity them " +
          quantity,
      );
      ...
      if (newCartItemQty > maxCartItemQuantity) {
        console.log(
          "[CartProvider - addProductToCart] Neu tang them " +
            quantity +
            ", quantity hien tai trong cart cua product " +
            productId +
            " se la " +
            newCartItemQty +
            ". Vuot qua " +
            maxCartItemQuantity +
            ". Stop update quantity cua product nay trong cart !",
        );
        ...
        return {
          ...
          addedProduct: null,
          errorMsg:
            "Số lượng của sản phẩm này trong giỏ hàng chỉ được phép tối đa là " +
            maxCartItemQuantity,
        };
      }
      ...
      console.log(
        "[CartProvider - addProductToCart] Data của updatedCartItem :",
      );
      ...
      setCart((currentCart) => {
        console.log(
          "[CartProvider - addProductToCart - setCart] Value hiện tại của biến useState cart : ",
        );
        console.log(currentCart);

        const updatedCart = currentCart.map((cartItem) => {
          console.log(
            "[CartProvider - addProductToCart - setCart] currentCart.map() - phần tử cartItem : ",
          );
          console.log(cartItem);

          if (cartItem.productId === updatedCartItem.productId) {
            console.log(
              "[CartProvider - addProductToCart - setCart] currentCart.map() - cartItem.productId = updatedCartItem.productId ! add updatedCartItem vào mảng mới ! ",
            );
            return updatedCartItem;
          }

          console.log(
            "[CartProvider - addProductToCart - setCart] currentCart.map() - cartItem.productId != updatedCartItem.productId ! add chính cartItem vào mảng mới ! ",
          );
          return cartItem;
        });

        console.log(
          "[CartProvider - addProductToCart - setCart] Data cua updatedCart :",
        );
      ...
      console.log(
        "[CartProvider - addProductToCart] Product co ID " +
          productId +
          " chua co trong cart. Bat dau add vao trong cart voi quantity = " +
          quantity,
      );
      ...
      if (!productObj) {
        console.log(
          "[CartProvider - addProductToCart] Product co ID " +
            productId +
            " khong ton tai trong mang productsList !",
        );
        ...
        return {
          ...
          addedProduct: null,
          errorMsg:
            "Sản phẩm này không tồn tại trong danh sách sản phẩm của tiệm bánh",
        };
      }
      ...
      if (quantity > maxCartItemQuantity) {
        console.log(
          "[CartProvider - addProductToCart] quantity chuan bi add vao cart cho product " +
            productObj.id +
            " la " +
            quantity +
            ". Vuot qua " +
      ...
      setCart((currentCart) => {
        ...
        console.log(
          "[CartProvider - addProductToCart - setCart] Data của newCart :",
        );
        console.log(newCart);
  ...
  useEffect(() => {
    ...
    return () => {
      console.log(
        "[CartProvider] đang chạy hàm cleanup của useEffect() test !",
      );
    };
  });
